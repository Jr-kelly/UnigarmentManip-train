...
from train.base.config import Config
from train.model.pointnet2_Sofa_Model import Sofa_Model
from train.val.simple_val import *
...
if __name__ == '__main__':
    ...
    # resume_path = "/home/user/DexGarmentLab-master/DexGarmentLab-master/Checkpoint/f2d/trousers/checkpoint_2_10000.pth"
    resume_path = "/home/user/DexGarmentLab-master/DexGarmentLab-master/Checkpoint/f2d/trousers_v2/trousers.pth"
    model=Sofa_Model(feature_dim=config.feature_dim)
    model.load_state_dict(torch.load(resume_path,map_location=device)['model_state_dict'])
    model = model.to(device)
    model.eval()
    ...
    for i in range (1, len(npz_paths)):
        ...
        print(f"idxs: {idxs}")
        # for idx in idxs:
        path = os.path.join(npz_path, f"p_{0}.npz")
        data = np.load(path)
